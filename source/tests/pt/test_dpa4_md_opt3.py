...
def test_fixed_edge_schema_uses_distributed_safe_sinks() -> None:
    ...
    neighbor_matrix = torch.tensor([[1, 0], [0, 1]], dtype=torch.long)
    num_neighbors = torch.tensor([1, 2], dtype=torch.long)
    shifts = torch.zeros((2, 2, 3), dtype=torch.long)

    schema = opt3._fixed_edge_schema_from_neighbor_matrix(
        coord=coord,
        atype=atype,
        cell=torch.eye(3, dtype=torch.float64).reshape(1, 3, 3) * 8.0,
...
def test_capture_scope_contains_builder_model_integrator_and_state_update() -> None:
    ...
    assert "nv_search_matrix_fixed" in build_source
...
def test_device_overflow_status_is_recorded_and_rejected() -> None:
    ...
    assert "increase" in check_source
